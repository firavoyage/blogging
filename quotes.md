...
- 避免对话
  - 只用第二人称和第三人称 `我` -> `少年` `友人` `画中人` `面前` `愚者`
  - 第二人称 `你` `戏外人` 第三人称 `祂` `天` `或` `之`
  - 旁白陈述语气 `如果你赢得了世界,我会很开心.` -> `你赢得...`
  - 上帝视角 `你心里一定想着...` `你可能要说` -> `你想...` `你说...`
  - 无心无波澜 `有人觉得` -> `或者说` 
  - 弱化转折 `但是` -> `这样` `此时` `只是` `无非` 
- 强化逻辑
  - 使用同位语 `a是b` -> `a b` `a,b.`
  ...
  - 激进关联 `不仅是 而且是` -> `不是 而是`
  - 双重否定 `一切` -> `没有 不`
  - 降低语速 `barrak obama address`
- 强化疏离感
  - 使用舶来词语 `日式` `广式` `台式`
  - 使用文言语法 `宾语前置` `定语后置`
- 简化表述
  - 只是关键词 `google search` `mistral chat`
  - 去除结构 保留内容 `把 a 定义为 1` `b 的值为 2` `给 c 赋值为 3` -> `a 1 b 2 c 3 (ref fp|oop)`
  - 避免重复 `反之` `亦然` `不然` `或然`
- fin
  - so much for this have a nice day.
