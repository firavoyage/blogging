...
- 避免对话 删去 `我` `你` `他` `她` `她` `它` 保留 `人们` `祂` `之`
- 避免对话 `可能就会问了/有人说/回答一些评论` -> `或者说`
- 弱化转折 `但是` -> `无非` `只是` `那么` `这样`
- 强化判断 `有` `要` `反映` `体现` -> `(同位语)` `是`
- 强化判断 `不仅是 而且是` -> `不是 而是`
- 强化判断 `都` `所有` `一切` -> `没有 不`
- 强化逻辑 `(删去一半字符,降低一倍语速,ref barrak obama address)`
- 强化逻辑 `反之` `不然` `亦然`
- 强化逻辑 去掉所有虚词 `的` `地` `得` `着` `了` `过`
- 强化疏离感 使用`日式`/`广式`/`台式`二字中文词语
- 强化疏离感 使用文言语法,`宾语前置`,`定语后置`
...
科技,大概是数学,英语...
或者说,也包括材料物化,生物医药,航空航天.
无非坡度平缓些的,编码和网路.

像一个人不爱好英文的,最好的翻译也不道地
...
- math
  - 会假设高考 120 分的能力,或者说 135,140,特别地
  - 练习中会发现许多不成文的定理
  - "一般地,n 个未知数 n 个式子,就可以解出."
  - "1..4 次方程,解析解.5 次以上反之."
- english
  - 高中英语水平.
