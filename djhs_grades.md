# djhs grades
## self
```
shanghai dajing vocational high school 6-28
graduate in 2026
```

> just let time be precious...

## grades

```
a+ 5% //#1~15
a b+ b b- c+ c c- d+ d 10%
e 5%
```

```
//number after name stands for class
//max 120 each

//grade x term y
//sum place chinese math english

//g1t1
sep 274   a+#1  76 b  101 a+  97   a+  //jiang 271 #2
oct 273   a+#2  69 c- 106 a+  98   a+  //someone7 274 #1 math 111
mid 251   a     62 d+  92 a   98   a+  //jiang 277.5 #4 67 119 91.5
dec 252.5 a     73 c+  80 b+ 101.5 a+  //nothing here
fin 275.5 a+#1  73 a  106 a+  96.5 a+  //xuruoxi 274 #2 | jiang 270 #3
//g1t2
feb 260.5 a+#12 75 b+  92 a   93.5 a   //lin #6
mar 289   a+#3  86 a+  98 a+ 105   a+  //jiang 296 #1 88 105 103 | yaoqixuan1 290.5 #2 72 105 112.5
mid 265.5 a+#14 80 a   90 b+  95.5 a   //jiang 283.5 #1 74 110 99.5 (10 subjects all a+ except ch b-)
may 254.5 a+#9  67 c-  92 a+  95.5 a+  //jiang 284.5 #1 77 104 103.5 | lvfuyuan 253 a+
fin 233   a     75 b+  80 a   78   a   //max 100 | jiang 255.5 #1 75 96 84.5 | lvfuyuan 235.5 a+ 67 98 70.5
```

## comments
20240117
grade 1 autumn finished!
> do well from start to finish #1
> its always day 1 ❤

2024 mar
room and seat 0721

2024 mid
room and seat 1024

2024 may
room and seat 0515
exam at date 0520!
> wangyining ❤ fira

2024 g1t2 fin
room and seat 0521

20240619
区统一考试结束了两门
语文,超出水平
祂只要多考我背诵篇目的原文,注释,翻译,就可以拿下我
不仅没有,还要考续写结尾,还要考"文学的光亮",不给辩题自拟观点
数学,大考经验不足
时间紧迫,先做题再读题,分类讨论了不符定义域的情况(最后都没有舍解)
压轴第一问,要用定义证明`sin2x+acosx (a < -2)`在`[0,2pi]`严增,任取...作差失败,还固执地在草稿纸求导试试看(浪费时间).
然后第二三问讨论三角函数零点情况,其实好做的,没时间了
> retro
不像中考数学,先做后面,最后都举重若轻地补全,2023上海,145.

20240620
英文,如常
听力有听写句子,报三遍,一三匀速,二每个分句停顿about 5s
作文"small changes make an ideal school library"

20240624
f:
我三门没上80的,有机会a+吗
陈:
你数学英语在干嘛?语文还行
f:
75,80,78
陈:
应该达不到你的预期
f:
按120算,jiang上次拉我30
这次27
陈:
我听说的年级里高分都比你高
f:(->jiang)
你这次虽然拉我少点
但不是断层了
相当于我更差了qwq

20240626
实际上
差1分a+


